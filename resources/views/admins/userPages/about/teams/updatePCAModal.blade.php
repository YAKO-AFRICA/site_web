<div class="modal fade" id="updatePCA{{$PCA->uuid}}" tabindex="-1" aria-labelledby="updateModalLabel{{$PCA->uuid}}" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="updateModalLabel{{$PCA->uuid}}">Mise à jour</h5>
                <button class="btn btn-close p-1" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.about.updateTeam', $PCA->uuid) }}" method="POST" class="submitForm" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        <div id="previewContainer" class="dz-preview d-flex flex-wrap border border-translucent bg-body-emphasis rounded-3 d-flex flex-center position-relative me-2 mb-2" style="height:140px;width:140px;">
                            ...
                            @if($PCA->team_image)
                                <img src="{{ asset('images/Teams/' . $PCA->team_image) }}" alt="Image" style="max-width: 100%; max-height: 100%;">
                            @endif
                        </div><br>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="team_name" value="{{$PCA->team_name}}" class="form-control" required autocomplete="off">
                    </div>
                    <!-- Fonction -->
                    <div class="mb-3">
                        ...
                        <select name="team_fonction" class="form-control" required autocomplete="off">
                            <option selected value="{{$PCA->team_fonction}}">{{$PCA->team_fonction}}</option>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control tinymce-editor" name="team_description" rows="3" autocomplete="off">{{$PCA->team_description}}</textarea>
                    </div>
                </div>
