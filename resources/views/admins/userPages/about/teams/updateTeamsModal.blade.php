<div class="modal fade" id="updateTeams{{$team->uuid}}" tabindex="-1" aria-labelledby="updateModalLabel{{$team->uuid}}" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="updateModalLabel{{$team->uuid}}">Mise à jour</h5>
                <button class="btn btn-close p-1" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.about.updateTeam', $team->uuid) }}" method="POST" class="submitForm" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        <div id="previewContainer" class="dz-preview d-flex flex-wrap border border-translucent bg-body-emphasis rounded-3 d-flex flex-center position-relative me-2 mb-2" style="height:140px;width:140px;">
                            ...
                            @if($team->team_image)
                                <img src="{{ asset('images/Teams/' . $team->team_image) }}" alt="Image" style="max-width: 100%; max-height: 100%;">
                            @endif
                        </div><br>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="team_name" value="{{$team->team_name}}" class="form-control" required autocomplete="off">
                    </div>
                    <!-- Fonction -->
                    <div class="mb-3">
                        ...
                        <select name="team_fonction" class="form-control" required autocomplete="off">
                            <option selected value="{{$team->team_fonction}}">{{$team->team_fonction}}</option>
