...
@section('content')
                ...
                <div class="col-12">
                    <div class="services__tab-wrap">
                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                            @foreach($products as $index => $product)
                                <li class="nav-item" role="presentation">
                                    <a class="nav-link {{ $index === 0 ? 'active' : '' }}" onclick="productVisitedForm('{{$product->uuid}}')" id="tab-product-{{ $product->uuid }}" data-bs-toggle="tab" data-bs-target="#tab-product-formul-{{ $product->uuid }}" type="button" role="tab" aria-controls="tab-product-formul-{{ $product->uuid }}" aria-selected="{{ $index === 0 ? 'true' : 'false' }}">{{ $product->label }}</a>
                                </li>
                            @endforeach
                        </ul>
                        ...
                        <div class="tab-content" id="myTabContent">
                            @foreach($products as $index => $product)
                                <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="tab-product-formul-{{ $product->uuid }}" role="tabpanel" aria-labelledby="tab-product-{{ $product->uuid }}" tabindex="0">
                                                ...
                                                <div class="row">
                                                    <div class="col-100">
                                                        <div class="blog-post-wrap">
                                                            <div class="row gutter-24" id="formules-{{ $product->uuid }}">
                                                                <!-- Les formules seront ajoutées dynamiquement ici -->
                                                            </div>
                                                        </div>
                                                        <div class="pagination-wrap mt-40">
                                                            <nav aria-label="Page navigation example">
                                                                <ul class="pagination list-wrap" id="pagination-{{ $product->uuid }}">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateFormules(productUuid) {
                ...
                if (formulesContainer) {
                    ...
                    const formules = formulesByProduct[productUuid] || [];
    ...
    <script>
        function productVisitedForm(uuid) {
            ...
            // Remplir le champ `product_uuid` avec la valeur passée
            ...
            .then(data => {
                ...
                // réenitialiser le champ product_uuid
        ...
        function formuleVisitedForm(uuid, redirectUrl) {
            ...
            // Remplir le champ `prodformule_uuid` avec la valeur passée
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const formulesByProduct = @json($formulesByProduct);
            const itemsPerPage = 2; // Nombre de formules par page
        
            // Fonction pour mettre à jour les formules avec pagination
            function updateFormules(productUuid, currentPage = 1) {
                const formulesContainer = document.getElementById(`formules-${productUuid}`);
                if (formulesContainer) {
                    formulesContainer.innerHTML = ''; // Vider le conteneur
        
                    const formules = formulesByProduct[productUuid] || [];
                    const totalPages = Math.ceil(formules.length / itemsPerPage);
        
                    // Afficher seulement les formules pour la page courante
                    const start = (currentPage - 1) * itemsPerPage;
                    const end = start + itemsPerPage;
                    const formulesToShow = formules.slice(start, end);
        
                    if (formulesToShow.length > 0) {
                        formulesToShow.forEach(formule => {
                            const formulaHtml = ` 
                                <div class="col-lg-6 col-md-6">
                                    <div class="blog__post-two shine-animate-item">
                                        <div class="blog__post-thumb-two">
                                        ...
                                        <div class="blog__post-content-two">
                                            ...
                                            <div class="card-footer m-auto" style="width: 50%;">
                                                <a href="javascript:void(0)" class="btn" onclick="formuleVisitedForm('${formule.uuid}', '{{ route('home.formule_product.details', '') }}/${formule.uuid}')">Voir Plus</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    updatePagination(productUuid, currentPage, totalPages);
                }
            }
        
            // Fonction pour mettre à jour la pagination
            function updatePagination(productUuid, currentPage, totalPages) {
                const paginationContainer = document.getElementById(`pagination-${productUuid}`);
                if (paginationContainer) {
                    paginationContainer.innerHTML = ''; // Vider la pagination existante
        
                    // Ajouter le bouton de page précédente
                    ...
                    // Ajouter le bouton de page suivante
                    ...
                    // Ajouter un événement pour gérer les clics sur les liens de pagination
                    paginationContainer.querySelectorAll('.page-link').forEach(link => {
                        link.addEventListener('click', (event) => {
                            event.preventDefault();
                            const page = parseInt(event.target.getAttribute('data-page'));
                            if (page >= 1 && page <= totalPages) {
                                updateFormules(productUuid, page);
                            }
                        });
                    });
                }
            ...
            // Fonction pour initialiser l'affichage des formules pour le premier produit
            function initializeFirstTab() {
                const firstTabLink = document.querySelector('.tab-pane.show.active');
                if (firstTabLink) {
                    const productUuid = firstTabLink.getAttribute('id').replace('tab-product-formul-', '');
                    updateFormules(productUuid);
                }
            }
